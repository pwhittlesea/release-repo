...
public class RepositoryFileSystem implements Repository {
	...
	public List<Resource> allResources(final String family, final String versionName) throws NotFoundException {
		...
		for (final Product product : products(family)) {
			final String productName = product.getName();
			for (final Version version : versions(family, productName)) {
				if (version.getName().equals(versionName)) {
					allResources.addAll(resources(family, productName, versionName));
				}
			}
		}
		return allResources;
	...
	@Override
	public Resource getResource(final String family, final String product, final String version, final String name) throws NotFoundException {
		final File resourceFile = new File(pathHelper.getResourcePath(family, product, version, name));
		if (resourceFile.isFile()) {
			return new FileResource(resourceFile);
		}
		throw new NotFoundException("Could not find resource: family[" + family + "] product[" + product + "] version[" + version + "] with name " + name);
	}

	@Override
	public List<Product> products(final String family) throws NotFoundException {
		final List<File> contents = getContentsOf(pathHelper.getFamilyPath(family), false);
		final List<Product> products = new ArrayList<Product>();
		for (final File file : contents) {
			products.add(new FileProduct(file));
		}
		return products;
	}

	@Override
	public List<Resource> resources(final String family, final String product, final String version) throws NotFoundException {
		final List<File> contents = getContentsOf(pathHelper.getVersionPath(family, product, version), true);
	...
	@Override
	public List<Version> versions(final String family, final String product) throws NotFoundException {
		final List<File> contents = getContentsOf(pathHelper.getProductPath(family, product), false);
